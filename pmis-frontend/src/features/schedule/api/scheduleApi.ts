...
export const getProjectSchedules = async (
  ...
  const response =
    await client.get<ScheduleResponse[]>(
 ...
 *
 * GET
 * /api/projects/2/schedules/calendar
 * ?startDate=2026-01-01
 * &endDate=2026-09-09
 */
export const getProjectScheduleCalendar = async (
  projectId: number,
  ...
  const response =
    await client.get<ScheduleResponse[]>(
      `${PROJECT_API}/${projectId}/schedules/calendar`,
      {
        params,
      },
    );
...
export const createSchedule = async (
  ...
  const response =
    await client.post<ScheduleResponse>(
      `${PROJECT_API}/${projectId}/schedules`,
      request,
    );
...
export const updateSchedule = async (
  ...
  const response =
    await client.put<ScheduleResponse>(
      `${SCHEDULE_API}/${scheduleId}`,
      request,
    );
